      ...
      <div className='container-md'>
          ...
          <div className='form-container d-flex' style={{background:"#F0F0F0", justifyContent:"center", padding:"15px"}}>
            <div>
              ...
              <input type="text" id="event-name" name="event-name" placeholder='Event Name' style={{width:"90%"}}  />
            </div>
            <div>
              <label htmlFor="location">Where it should be?</label>
              <input type="text" id="location" name="location" placeholder="Event Location"style={{width:"90%"}}  />
            </div>
            <div>
              <label htmlFor="date">On Which Date?</label>
              <input type="date" id="date" name="date" placeholder='Pick a day' style={{width:"90%"}}  />
            </div>
          </div>
        </div>
        <div className='row'>
          ...
          {events && events.map((event, index) => (
            <div className='card' key={index} style={{ maxWidth:"300px", margin:"auto", marginBottom:"10px", Height:"350px"}}>
              <img className='card-img-top' style={{height:"200px"}} src={event.poster_url} alt={event.event_name} />
